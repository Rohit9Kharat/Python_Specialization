...
conn = sqlite3.connect('all_nz_geodata.sqlite')
cur = conn.cursor()

cur.execute('''
CREATE TABLE IF NOT EXISTS NZ_Cities (address TEXT, country TEXT, state TEXT, district TEXT, city TEXT, postal_code TEXT, latitude TEXT, longitude TEXT, geodata TEXT)''')
...
fh = open("where_nz.data", encoding="utf8")
count = 0
...
for line in fh:
    ...
    address = line.strip()
    print('')
    cur.execute("SELECT geodata FROM NZ_Cities WHERE address= ?",
    ...
    cur.execute('''INSERT INTO NZ_Cities (address, country, state, district, city, postal_code, latitude, longitude, geodata)
