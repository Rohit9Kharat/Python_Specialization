...
conn = sqlite3.connect('all_geodata.sqlite')
cur = conn.cursor()

cur.execute('''
CREATE TABLE IF NOT EXISTS Universities (address TEXT, country TEXT, state TEXT, district TEXT, city TEXT, postal_code TEXT, latitude TEXT, longitude TEXT, geodata TEXT)''')
...
for line in fh:
    ...
    address = line.strip()
    print('')
    cur.execute("SELECT geodata FROM Universities WHERE address= ?",
    ...
    cur.execute('''INSERT INTO Universities (address, country, state, district, city, postal_code, latitude, longitude, geodata)
